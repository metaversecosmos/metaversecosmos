<div align="center">

# ⚛️ Hi, I'm Satoshi Qore 👋

### Web3 & Blockchain Infrastructure Enthusiast

<img src="https://raw.githubusercontent.com/metaversecosmos/metaversecosmos/main/banner.png" width="100%" />

---

</div>

<br/>

## 🧬 About Me

I’m interested in Web3 and blockchain infrastructure.

I learn by building and running real systems like QoreChain node setups.

<br/>

---

## 🚀 What I Do

<div>

- ⚙️ Run blockchain nodes (QoreChain and test networks)  
- 🧪 Experiment with Web3 infrastructure tools  
- 🧠 Learn by setting up and operating real systems  
- 🐳 Work hands-on with Docker and CLI tools  

</div>

<br/>

---

## 🧩 Focus Areas

<div>

- Node operations and deployment  
- Decentralized infrastructure systems  
- Learning by doing (not just theory)  

</div>

<br/>

---

## 🛠️ Tools

<div>

Docker • CLI • Windows • Linux • GitHub

</div>

<br/>

---

## 📡 Status

<div align="center">

🟢 Learning Web3 Infrastructure  
🧪 Running node experiments  
⚙️ Building hands-on system experience  
🔁 Continuous learning loop: build → break → fix  

</div>

<br/>

---

## 🌍 Connect

- GitHub: https://github.com/metaversecosmos  
- X: https://x.com/satoshi_Qore  

<br/>

---

<div align="center">

> “I learn systems by running them.”

</div>
